...
conn.execute("CREATE TABLE test_table (i INTEGER, j VARCHAR)")
...
print(duckdb.filter(test_df, 'i > 1').order('j').project('i + 1').limit(2))
...
res = duckdb.query('SELECT * FROM test_df', 'test_df')
print(res.df())
